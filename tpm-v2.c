/* O κώδικας σε C για την υλοποίηση της εντολής tpm_nv_read_value καθώς και άλλων.
Βρίσκεται στην θέση ~/u-boot/cmd/tpm-v2.c του Raspberry Pi 3 B  
*/
...
/************** υλοποίηση της εντολής tpm_nv_read_value ***********************/

static int do_tpm_nv_read_value(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
	u32 index, count, rc;
	struct udevice *dev;
	void *data;
...
/********** τέλος υλοποίηση της εντολής tpm_nv_read_value *****************/
...
static struct cmd_tbl tpm2_commands[] = {
	...
	U_BOOT_CMD_MKENT(pcr_read, 0, 1, do_tpm_pcr_read, "", ""),
        U_BOOT_CMD_MKENT(nv_read_value, 0, 1, do_tpm_nv_read_value, "", ""),   // and this
